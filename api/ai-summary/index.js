...
module.exports = async function (context, req) {
  ...
  const includeBudgetEstimate = payload.proposal && payload.proposal.budgetEstimateInput
    ...
    : true;
  const waveInstruction = "Si proposal.adoptionWaveModel o proposal.budgetEstimateInput.adoptionWaveModel existe, explicar el alcance con ese modelo de olas. Si el precio es por ola, no presentarlo como total cerrado para todos los usuarios; decir explicitamente que es referencia por ola y que el total final depende de cantidad de olas/add-ons.";
  const addonInstruction = "Cowork no debe presentarse como oferta suelta: requiere Microsoft 365 Copilot activo/trial y adopcion minima, y se describe como add-on de 3 a 5 sesiones o siguiente paso posterior a P2. El Webinar introductorio Copilot en 30 minutos es una entrada liviana de sensibilizacion con ejemplos generales, moderacion y Q&A; no reemplaza un programa de adopcion. Los webinars especializados de agentes, agentes en SharePoint, notebooks o funcionalidades Copilot son consumos puntuales por tecnologia, no sustituyen P1/P2/P3.";
  const progressionInstruction = "Si proposal.progressionGuidance existe, respetarlo como guardrail comercial: explicar cuando una demanda esta adelantada, que prerrequisito falta y cual es el siguiente escalon recomendado.";
  const p0Instruction = "Para P0/Envisioning IA, mantener add-ons acotados: solo Casos para lideres, Readiness tecnico y Webinar introductorio Copilot en 30 minutos. Si el cliente necesita mas modulos, sugerir cambiar a P1, P2 o P3 segun corresponda.";
  const p1Instruction = "Para P1/Productividad Digital, no describirlo como capacitacion puntual. Presentarlo como programa de adopcion Microsoft 365 con diagnostico, comunicacion, formacion aplicada, acompanamiento y medicion; las sesiones son un componente del recorrido, no el producto completo. En P1 solo son add-ons normales Readiness tecnico, Excel intermedio, Excel avanzado y Modulo a medida de herramienta puntual; si el cliente necesita mas, sugerir ampliar alcance o cambiar a P2/P3.";
  const p2Instruction = "Para P2/IA aplicada al trabajo, no sumar modulos de productividad base ni cambio organizacional como add-ons normales. P2 ya incluye casos por rol, Prompt Library, Copilot en Excel y medicion; son add-ons normales Cowork y Copilot, Agents & Cowork, Agentes a medida con Copilot Studio, Webinar especializado de agentes, Webinar especializado de agentes en SharePoint, Webinar especializado de notebooks/funcionalidades Copilot, PEAT Discovery/Build, Gobierno de IA y agentes y Foundry/Fabric/Data Readiness cuando el caso excede adopcion Copilot estandar.";
  const p3Instruction = "Para P3/Productividad Digital + IA, tratarlo como programa integral con adopcion Microsoft 365, Copilot, casos por rol, Prompt Library, medicion, comunicacion, champions, gestion del cambio y webinars incluidos. Cowork y Copilot, Agentes a medida con Copilot Studio, PEAT Discovery/Build, Gobierno de IA y agentes y Foundry/Fabric/Data Readiness son add-ons seleccionables/cotizables aparte segun alcance; no presentarlos como incluidos por defecto.";
  const includedVsAddonInstruction = "Distinguir siempre componentes incluidos del plan base versus add-ons/opcionales cotizables. Lo incluido no es un add-on por defecto, no se selecciona ni se suma al precio; forma parte del alcance base del plan. La modalidad virtual esta incluida; si aparece un deliveryCharge por modalidad presencial o mixta, tratarlo como adicional operativo automatico y sumarlo al presupuesto, no como modulo funcional opcional.";
  const responseFormat = includeBudgetEstimate
    ? `Devolver texto en Markdown con secciones: ## Contexto de la empresa, ## Lectura comercial, ## Recomendacion Possumus, ## Estimacion asistida de presupuesto, ## Argumentos para la reunion. En Contexto de la empresa incluir rubro, productos/servicios, escala si esta sustentada y canales/contactos publicos si existen. En Recomendacion Possumus respetar proposal.recommendedPlanDuration, proposal.recommendedPlanScope y adoptionWaveModel. En Estimacion asistida de presupuesto usar proposal.budgetEstimateInput: si shouldEstimate es false, no cerrar presupuesto y pedir validacion con preventa/adopcion; si es true, indicar importe sugerido, modalidad comercial, supuestos y alertas. ${waveInstruction} ${progressionInstruction} ${includedVsAddonInstruction} ${p0Instruction} ${p1Instruction} ${p2Instruction} ${p3Instruction} ${addonInstruction} No inventar precios para add-ons a cotizar. La ultima seccion debe usar bullets.`
    : `Devolver texto en Markdown con secciones: ## Contexto de la empresa, ## Lectura comercial, ## Recomendacion Possumus, ## Argumentos para la reunion. En Contexto de la empresa incluir rubro, productos/servicios, escala si esta sustentada y canales/contactos publicos si existen. En Recomendacion Possumus respetar proposal.recommendedPlanDuration, proposal.recommendedPlanScope y adoptionWaveModel. ${waveInstruction} ${progressionInstruction} ${includedVsAddonInstruction} ${p0Instruction} ${p1Instruction} ${p2Instruction} ${p3Instruction} ${addonInstruction} No incluir estimacion de presupuesto porque el comercial la desactivo. La ultima seccion debe usar bullets.`;
